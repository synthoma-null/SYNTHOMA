import type { CyklusRunState, SectorId, PackCardRole } from './cyklusTypes';
...
export function getStoryDirective(state: CyklusRunState, story: StoryProgression): StoryDirective {
  const base = ACT_DIRECTIVES[story.currentAct] ?? ACT_DIRECTIVES.act1_sandbox_glitchka;
  ...
  if (thread) {
    return {
      ...base,
      preferredTags: [...base.preferredTags, ...thread.preferredTags],
      preferredSectors: thread.preferredSector ? [thread.preferredSector, ...base.preferredSectors] : base.preferredSectors,
    };
  }
...
export function applyStoryScore(
  ...
  score: number,
  card: { packId?: string; tags: string[]; sector?: SectorId; role?: string },

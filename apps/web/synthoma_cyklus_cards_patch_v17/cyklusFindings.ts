...
export const CYKLUS_FINDINGS: DiagnosticFinding[] = [
  ...
  {
    id: 'form_office_regular',
    title: 'Pravidelný klient Formulářovny',
    description:
      'Subjekt se opakovaně dotkl formulářů, podpisů nebo úředních dveří. Systém jej klasifikuje jako administrativně kontaminovaného. Což je horší než kletba, protože to má přílohu.',
    check: (s) =>
      s.inventory.includes('blank_form') ||
      s.inventory.includes('rubber_stamp') ||
      ['choose_form_office', 'form_smiles', 'form_misunderstands', 'form_collects_signature', 'audit_siren'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'form_office_pool' },
  },
  {
    id: 'unfinished_thread_keeper',
    title: 'Správce nedokončených vět',
    description:
      'Subjekt nechal rozhovor viset ve vzduchu tak dlouho, až z něj vznikl stavební materiál. V SYNTHOMĚ tomu říkáme vztahová architektura, protože „problém“ znělo moc upřímně.',
    check: (s) =>
      s.imprints.includes('unfinished_conversation') ||
      s.flags.includes('unfinished_conversation_active') ||
      s.usedCardIds.includes('unfinished_conversation'),
    reward: { unlockPool: 'relationship_followups' },
  },
  {
    id: 'sarkasma_accountant',
    title: 'Sarkasmin dlužník s účtenkou',
    description:
      'Subjekt nashromáždil Sarkasmin dluh nebo účtenku. Humor ho podržel. Humor si to poznamenal. Humor je v tomhle světě účetní s hezkým nožem.',
    check: (s) =>
      s.imprints.includes('sarkasma_debt') ||
      s.inventory.includes('sarkasma_receipt') ||
      s.flags.includes('sarkasma_debt') ||
      ['sarkasma_account', 'sarkasma_invoice', 'sarkasma_collects'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'sarkasma_debt_pool' },
  },
  {
    id: 'token_namer',
    title: 'Kmotřič rezavých žetonů',
    description:
      'Subjekt pojmenoval nebo proměnil žeton. Kov se tím nestal živým. Jen začal mít nepříjemně osobní nároky, což je prakticky totéž.',
    check: (s) =>
      s.inventory.includes('named_token') ||
      s.inventory.includes('warm_token') ||
      ['token_wants_name', 'token_market_door', 'token_collector'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'token_market_pool' },
  },
  {
    id: 'boundary_merchant',
    title: 'Obchodník s odmítnutím',
    description:
      'Subjekt nechal tržiště ohmatat vlastní „ne“. Některé hranice se po prodeji vracejí silnější. Některé jen s účtenkou a pachutí ponížení.',
    check: (s) =>
      s.inventory.includes('returned_no') ||
      ['market_sells_your_no', 'someone_buys_your_no', 'replacement_problem'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'market_sells_no_pool' },
  },
  {
    id: 'wrong_name_cartographer',
    title: 'Kartograf špatných jmen',
    description:
      'Subjekt prošel místem, které ho oslovilo nesprávně, a nevybuchl. Identita sice protestovala, ale formulář vypadal spokojeně. Formuláře mají nízké nároky na morálku.',
    check: (s) =>
      s.flags.includes('wrong_name_returns') ||
      s.flags.includes('wrong_name_unanswered') ||
      ['sector_wrong_name', 'wrong_name', 'wrong_name_returns'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'wrong_name_pool' },
  },
  {
    id: 'soft_bug_keeper',
    title: 'Pečovatel o měkkou chybu',
    description:
      'Subjekt si ponechal chybu, která byla až podezřele hebká. Systém doporučuje opatrnost. Glitchka doporučuje deku. Obě doporučení jsou znepokojivě rozumná.',
    check: (s) =>
      s.inventory.includes('soft_bug') ||
      s.inventory.includes('named_soft_bug') ||
      ['soft_bug_grows', 'soft_bug_followup'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'soft_bug_pool' },
  },
  {
    id: 'beast_mark_survivor',
    title: 'Označen paměťovou šelmou',
    description:
      'Subjekt nese značku šelmy nebo ji aspoň viděl dost zblízka na to, aby měl horší dny. Paměťová šelma si nehraje na boss fight. Ona si hraje na dědictví.',
    check: (s) =>
      s.inventory.includes('memory_beast_mark') ||
      ['entity_memory_beast', 'memory_beast_returns'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'memory_beast_pool' },
  },
  {
    id: 'archive_forbidden_reader',
    title: 'Čtenář zakázaného regálu',
    description:
      'Subjekt otevřel věc, která měla zůstat zavřená. To není odvaha ani hloupost. Je to obvyklý lidský kompromis mezi obojím, jen s horším osvětlením.',
    check: (s) =>
      s.flags.includes('forbidden_archive_opened') ||
      s.inventory.includes('black_folder') ||
      ['black_folder', 'black_folder_rustles', 'key_in_wrong_office'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'archive_forbidden_pool' },
  },
  {
    id: 'sealarium_regular',
    title: 'Návštěvník Sealaria',
    description:
      'Subjekt vstoupil do gumově-tuleňového ekosystému. Nikdo neví, proč existuje. Všichni vědí, že jednou zachrání cyklus a bude se u toho tvářit uraženě.',
    check: (s) =>
      s.inventory.includes('rubber_seal') ||
      ['rubber_seal_squeaks', 'seal_squeaks', 'seal_demands_stamp', 'seal_wants_stamp_again'].some((id) => s.usedCardIds.includes(id)),
    reward: { unlockPool: 'sealarium_pool' },
  },
  {
    id: 'pool_archaeologist',
    title: 'Archeolog odemčených sektorů',
    description:
      'Subjekt odemkl alespoň šest poolů v jednom průchodu. Systém to nazval pokrokem. Všichni ostatní by tomu řekli sběr podezřelých dveří.',
    check: (s) => s.unlockedPools.length >= 6,
    reward: { title: 'Poolová kartografie' },
  },

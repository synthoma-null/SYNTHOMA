...
export default function HomePage() {
  ...
  // Hero INFO typing state (mirror Reader)
  ...
  // Hold the full original INFO HTML so that choices can jump into hidden sections (#story-cache)
  const infoFullHtmlRef = useRef<string>("");
  // Transform incoming HTML: convert <p class="choice"> to Reader-like <button class="choice-link">
  const transformChoicesToButtons = (html: string): string => {
    try {
      const root = document.createElement('div');
      root.innerHTML = html;
      const nodes = Array.from(root.querySelectorAll('p.choice')) as HTMLElement[];
      nodes.forEach((p) => {
        // If author explicitly provided an anchor link inside, keep it as a real navigation link
        // Example used in SYNTHOMAINFO.html: <p class="choice"><a class="choice-link" href="/books">…</a></p>
        const existingAnchor = p.querySelector('a.choice-link[href]');
        if (existingAnchor) {
          return; // do not replace with button; keep native link behavior
        }
  ...
  const setInfoFromHtml = (html: string) => {
    // normalize choices to buttons so they behave like in Reader
    const normalized = transformChoicesToButtons(html);
    setInfoHtml(normalized);
    try {
      const d = document.createElement('div');
      d.innerHTML = normalized;
      // Exclude hidden caches so we don't type forever
      d.querySelectorAll('#story-cache, .hidden').forEach(el => el.remove());
      // Keep raw text for potential debugging
      const plain = (d.textContent || '');
      setInfoPlain(plain);
      // Compute exact total length by summing text node lengths (matches revealed slicing)
      let total = 0;
      const walk = (node: Node) => {
        if (node.nodeType === Node.TEXT_NODE) { total += (node.nodeValue || '').length; return; }
        const kids = node.childNodes; for (let i = 0; i < kids.length; i++) walk(kids[i]);
      };
      walk(d);
  ...
  useEffect(() => {
    // Po zobrazení hero INFO načti HTML a připrav progressive reveal stejně jako v Readeru
  ...
  // Handle clicks on .choice within hero INFO by loading the target section from #story-cache
  useEffect(() => {
    if (!showHeroInfo) return;
    ...
    const onClick = (ev: MouseEvent) => {
      ...
      if (!t) return;
      // 0) Prefer client-side navigation for anchors
      ...
      // 1) Handle synthetic Reader-like buttons
      const p = t.closest('button.choice-link') as HTMLElement | null;
      if (!p) return;
      if (infoIsTyping) {
        ev.preventDefault(); ev.stopPropagation();
        // Finish typing immediately and proceed with the same click
        try { setInfoIsTyping(false); setInfoTypedCount(infoTotalLen); } catch {}
        // Do not return; continue to handle the choice in the same tick
      }
      ev.preventDefault(); ev.stopPropagation();
      const next = p.getAttribute('data-next') || '';
      if (!next) return;
      try {
        ...
        if (section) {
          // 1) Lock and style choices like Reader
          const hostEl = document.getElementById('hero-info') as HTMLElement | null;
          if (hostEl) {
            // Lock only the local choice group that contains the clicked button
            ...
            // 2) Append the target section content below the choices
            ...
            // Keep choices as-is and inject the next content under them with progressive reveal of final HTML
            const normalized = transformChoicesToButtons(section.innerHTML);
            // Wrapper renders directly the progressively revealed FINAL HTML
            const wrapper = document.createElement('div');
            wrapper.className = 'hero-followup-block';
            // Do NOT clear previous follow-ups; always append
            follow.appendChild(wrapper);

            // Compute plain text from normalized HTML
            ...
            // Progressive reveal: render final HTML structure and trim text node contents to current count
            ...
            const renderRevealed = (srcHtml: string, count: number): string => {
              try {
                ...
                // Hide empty choice buttons (no revealed text yet)
            ...
            // disabled: do not auto-scroll after injecting follow-up content
            // try { follow.scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch {}
          } else {
            // Fallback: replace content if host not found
  ...
  }, [showHeroInfo, infoHtml, infoIsTyping, infoPlain]);

  // Progressive reveal of HERO INFO final HTML (matches follow-up renderer)
  const revealedInfoHtml = useMemo(() => {
    ...
    try {
      ...
      Array.from(container.childNodes).forEach(processNode);
      // Hide empty choice buttons while their text hasn't been revealed
      container.querySelectorAll('button.choice-link').forEach((btn) => {
        const t = (btn.textContent || '').replace(/\s+/g, ' ').trim();
        if (!t) btn.classList.add('choice-empty');
      });
  ...
  // Tick typing like Reader (rychlost 3ms/znak)
  useEffect(() => {
    if (!showHeroInfo) return;
  ...
  // Staggered fade-in for choices once typing finishes (mirror Reader)
  useEffect(() => {
    if (!showHeroInfo) return;
    if (infoIsTyping) return;
  ...
  useEffect(() => {
    ...
    cancelDialog = runTypewriter({
      ...
      onStart: () => { /* efekt glitch řeší UI helpery, zde jen type */ },
  ...
  useEffect(() => {
    ...
    const scrollTerminalBottom = () => {
      // disabled: do not auto-scroll during typewriter
      return;
    };
  ...
  const handleContinue = async () => {
    ...
    // místo okamžitého přechodu do readeru zobraz hero INFO
    setBtnVisible(false);
    setShowHeroInfo(true);
    try { const w: any = window as any; if (w.stopShinning) w.stopShinning(); } catch {}
    // posuň na HERO INFO, ať to vypadá, že se něco sakra děje
  ...
  // Make CTA button actually fade-in once typing is done
  useEffect(() => {
    if (typedDone) {
  ...
  return (
    <main className="home" role="main">
      {/* background video */}
      {/* background video moved to RootLayout for global presence */}

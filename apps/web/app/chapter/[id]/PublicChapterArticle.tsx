  ...
  return (
    <main className="story public-chapter-page" id="main-content">
      ...
      <nav className="public-chapter-navigation" aria-label="Navigace kapitolami">
        {chapter.previousId ? <Link href={`/chapter/${chapter.previousId}`}>PŘEDCHOZÍ KAPITOLA</Link> : <span />}
        <Link href="/books">KNIHOVNA</Link>
        {chapter.nextId ? <Link href={`/chapter/${chapter.nextId}`}>DALŠÍ KAPITOLA</Link> : <span />}
      </nav>
      <p className="public-machine-links">
        <a href={`/ai/cs/chapters/${chapter.id}.md`}>MARKDOWN</a>
        {' // '}
        <a href={`/api/public/v1/chapters/${chapter.id}?locale=cs`}>JSON</a>
      </p>
    </main>
  );
}

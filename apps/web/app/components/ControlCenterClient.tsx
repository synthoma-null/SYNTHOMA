...
import { resetUiPreferences, updateUiPreferences, type MotionMode, type TextEffectsMode, type TypewriterSpeed } from '../../src/lib/uiPreferences';
import ThemeShopClient from './ThemeShopClient';

type TabId = 'display' | 'motion' | 'reading';

const TEXT = {
  cs: {
    title: 'OVLÁDACÍ CENTRUM', close: 'Zavřít nastavení', done: 'Hotovo', reset: 'Obnovit výchozí',
    display: 'Vzhled', motion: 'Pohyb', reading: 'Čtení', language: 'Jazyk', theme: 'Motiv',
  ...
  en: {
    ...
    display: 'Display', motion: 'Motion', reading: 'Reading', language: 'Language', theme: 'Theme',
...
export default function ControlCenterClient() {
  ...
  const tabs: TabId[] = readingContext ? ['display', 'motion', 'reading'] : ['display', 'motion'];
    ...
    <div id="control-panel" ref={panelRef} className={`control-panel control-center${open ? ' visible' : ''}`} role="dialog" aria-modal="true" aria-labelledby="control-center-title" aria-hidden={!open}>
      ...
      <div className="control-center__body">
        {tab === 'display' ? <section id="control-tabpanel-display" role="tabpanel" aria-labelledby="control-tab-display">
          ...
          <label className="control-center__range"><span>{copy.font}</span><input type="range" min="0.8" max="1.4" step="0.05" value={preferences.fontScale} onChange={(event) => updateUiPreferences({ fontScale: Number(event.target.value) })} /><output>{Math.round(preferences.fontScale * 100)}%</output></label>
          <label className="control-center__range"><span>{copy.opacity}</span><input type="range" min="0" max="1" step="0.05" value={preferences.readerOpacity} onChange={(event) => updateUiPreferences({ readerOpacity: Number(event.target.value) })} /><output>{Math.round(preferences.readerOpacity * 100)}%</output></label>
          <Toggle label={copy.glass} pressed={preferences.glassEnabled} onChange={(glassEnabled) => updateUiPreferences({ glassEnabled })} />
          <label className="control-center__range"><span>{copy.blur}</span><input type="range" min="0" max="24" step="1" value={preferences.glassBlur} disabled={!preferences.glassEnabled} onChange={(event) => updateUiPreferences({ glassBlur: Number(event.target.value) })} /><output>{preferences.glassBlur}px</output></label>
        </section> : null}
